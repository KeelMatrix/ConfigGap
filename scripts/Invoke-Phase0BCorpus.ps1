...
try {
    foreach ($repository in $index.repositories) {
        ...
        $repositoryUrl = if ($repository.url -match '(?i)\.git$') { $repository.url } else { $repository.url.TrimEnd('/') + '.git' }
        $clone = Invoke-GitCommand @('clone', '--filter=blob:none', '--depth', '1', '--no-tags', '--no-checkout', $repositoryUrl, $target) $cloneTimeoutSeconds
