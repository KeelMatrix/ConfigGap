...
try {
    ...
    Invoke-InspectorExpectFailure -MutatedPackagePath $mutatedPackage -MutatedSymbolsPath $SymbolsPath -ExpectedMessage '(?i)artifact' -CaseName 'unexpected archive entry'
    ...
    Invoke-InspectorExpectFailure -MutatedPackagePath $PackagePath -MutatedSymbolsPath $mutatedSymbols -ExpectedMessage '(?i)symbol' -CaseName 'missing required symbol entry'
