...
On Windows with `LongPathsEnabled=True`, the script owns long-path behavior for its clone, fetch, and checkout commands by invoking Git with `-c core.longpaths=true`. The preflight line and `metrics.json` record the exercised mode, configured and effective `core.longpaths`, `LongPathsEnabled`, and the resolved scratch-root length; no scratch-root length is treated as a universal safety limit. When the host cannot support the required paths, clone preflight fails closed with a named `CONFIGGAP_*` diagnostic. The report includes supported-domain recall, all-labeled-static-key recall, the controlled precision protocol, dynamic-blocking count, status for every repository, and load-failure count.
...
Before treating a Phase 0B report as current evidence, run `pwsh -NoProfile -File .\scripts\Test-Phase0BEvidence.ps1`. It fails closed unless the report names exact 40-hex `Code candidate ref` and `Evidence checkpoint ref` commits, the evidence checkpoint resolves in the repository as an evidence-only child of the code candidate, and the checkpoint is an ancestor of the checked-out `HEAD`.
