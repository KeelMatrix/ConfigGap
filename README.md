...
Windows has local performance and targeted test evidence. The repository workflow exercises Release build, full tests, package, and consumer smoke on its configured operating-system matrix. Linux has a reproducible container check; macOS remains unverified locally. See [platform support](docs/platform-support.md) for the matrix, reproducible command, and residual uncertainty.
...
These literal reads remain supported only when the receiver's root provenance is statically established: a root configuration type, a known framework root `Configuration` property, a local/field/property initialized from a proven root, or a same-compilation helper parameter with at least one visible call site whose arguments are all proven roots. Bounded ASP.NET Core activation recognizes conventional `Startup` constructors, `ControllerBase`-derived constructors, and constructors of concrete services explicitly registered through the framework `AddScoped`, `AddSingleton`, or `AddTransient` methods in the same compilation. Outside those explicit framework cases, a parameter with no visible same-compilation call site has no proven root provenance and produces `CG900`; absence of a caller is never treated as root evidence. A section or otherwise unproven parameter/property/field likewise produces `CG900` instead of an invented root key. Local aliases initialized directly from `GetSection("Section")` retain their section scope.
...
- [Analysis resource benchmark](docs/analysis-core-benchmark.md)
